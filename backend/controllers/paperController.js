...
const viewPaper = async (req, res) => {
    try {
        ...
        const encodedFilename = encodeURIComponent(paper.title.replace(/[^\w\s.-]/g, '') + '.pdf');

        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Length', stat.size);
        res.setHeader('Content-Disposition', `inline; filename="${paper.title}.pdf"; filename*=UTF-8''${encodedFilename}`);
